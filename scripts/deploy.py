...
def ensure_hostonly_net():
    ...
    run([find_exe("VBoxManage"), "hostonlyif", "create"], check=False)
...
def get_vm_settings_path(name: str) -> str:
    """Get the VM settings directory from VirtualBox."""
    r = subprocess.run(
        [find_exe("VBoxManage"), "showvminfo", name, "--machinereadable"],
...
def create_vm(name: str, ram: int):
    ...
    # Get VM settings directory for disk path
    vm_dir = get_vm_settings_path(name)
    vdi = os.path.join(vm_dir, f"{name}.vdi")
...
def download_iso() -> str:
    ...
    print_info(f"Downloading Talos ISO (~{81}MB)...")
    downloader = shutil.which("curl")
    if not downloader:
        print("  ✗ curl not found")
        sys.exit(1)

    run([downloader, "-f", "-L", "-o", ISO_NAME, ISO_URL], check=False, timeout=300)
